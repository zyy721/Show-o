...
from training.nuscenes_dataset import nuscenesDataset, nuscenesDatasetVal
...
if __name__ == '__main__':
    ...
    if config.mode == 'inpainting':
        ...
        dataset_imagenet = nuscenesDataset(
            ...
            "data/nuscenes/nuscenes_infos_val_temporal_v3_scene.pkl",

            image_size=256,

        )

        train_dataloader_t2i = DataLoader(dataset_imagenet, batch_size=config.training.batch_size,
        ...
        for batch in train_dataloader_t2i:
            ...
            pixel_values_past = pixel_values[:, :F_past].contiguous().view(-1, *pixel_values.shape[2:])
            ...
            images_gt = torch.clamp((images + 1.0) / 2.0, min=0.0, max=1.0)
            images_gt *= 255.0
            images_gt = images.permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
            pil_images_gt= [Image.fromarray(image_gt) for image_gt in images_gt]

            pil_images[0].save('tmp_0.jpg')
            pil_images[1].save('tmp_1.jpg')
            pil_images[2].save('tmp_2.jpg')
